...
class DataStorage {
	...
	deleteLocalDB = async (identifier) => {
	...
	fetchRadioBases = async (isStorage) => {
		...
		return isStorage !== 'true'
			? fetch(`${this.API_URL}/mapa/data_radiobase`, {
					method: 'GET',
					headers: {
			...
			? (() => {
					...
					switch (field) {
						case 'nom_sit':
							return db.otecel_
								.where('nom_sit')
								.startsWithIgnoreCase(infoSearch)
								.each((obj) => {
				...
				if (count > 0) {
					...
					return new Promise(function(resolve, reject) {
						...
						fetch(`${API_URL}/mapa/data_radiobase`, {
							...
							.then((response) => response.json())
							.then((datosnuev) => resolve(datosnuev.jsonData))
