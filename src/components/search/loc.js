...
class SearchFields extends React.Component {
	...
	getSuggestions(value) {
		...
		return this.state.RadioBases.filter(
			(user) =>
				regex.test(user.nom_sit) ||
				regex.test(user.cell_id) ||
				regex.test(user.dir) ||
				regex.test(user.parroquia)
		);
	}
	...
	renderSuggestion(suggestion) {
		return (
			<span>
				{suggestion.nom_sit}/{suggestion.cell_id}/{suggestion.dir}/{suggestion.parroquia}
			</span>
		);
	}
	...
	onestChange = (event, { newValue }) => {
		...
		newValue.length >= 2 &&
			...
			DataStorageSuggestNom_Sit.fetchRadioBasesSuggest(
				newValue,
				filterOptions,
				'nom_sit',
	...
	oncell_idChange = (event, { newValue }) => {
		...
		newValue.length >= 2 &&
			...
			DataStorageSuggestCell_Id.fetchRadioBasesSuggest(
				newValue,
				filterOptions,
				'cell_id',
				window.localStorage.getItem('acceptAdvisement')
	...
	ondireccionChange = (event, { newValue }) => {
		...
		newValue.length >= 2 &&
			...
			DataStorageSuggestDir.fetchRadioBasesSuggest(
				newValue,
				filterOptions,
				'dir',
				window.localStorage.getItem('acceptAdvisement')
	...
	onparroquiaChange = (event, { newValue }) => {
		...
		newValue.length >= 2 &&
			...
			DataStorageSuggest.fetchRadioBasesSuggest(
				newValue,
				filterOptions,
				'parroquia',
				window.localStorage.getItem('acceptAdvisement')
